...
// ncclDevFuncTable removed — host-side dispatch eliminates need for device-side table
...
__device__ void ncclKernelMain(struct ncclDevComm* comm, uint64_t channelMask, struct ncclWork* workHead) {
  ...
  while (true) {
    ...
    // Host-side dispatch picks the right specialized kernel.
    // No device-side function table dispatch needed.
