...
// Host-side table of kernel function pointers — now dynamically populated by
// per-operation .so registration.
#define NCCL_MAX_DEV_KERNELS 2048
#define NCCL_MAX_DEV_FUNCS   2048

// Launch function type — each per-op .so provides one launch function
typedef void (*ncclOpLaunchFunc_t)(void* kernelFn, dim3 grid, dim3 block, void** args, size_t smem, cudaStream_t stream);

extern int ncclDevKernelCount;
...
// Dynamic registration API — called by each per-op .so at load time.
...
extern ncclOpLaunchFunc_t ncclOpLaunchFn;  // current launch function (set per-plan)
