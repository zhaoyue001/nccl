...
static void ncclLoadOps() {
  ...
  const char* searchPaths[] = {
    getenv("NCCL_OPS_DIR"),
    "build/ops",
    "/root/nccl-2.20.5/build/ops",
    NULL
  };
  
  DIR* dir = NULL;
  const char* opsDir = NULL;
  for (int i = 0; searchPaths[i]; i++) {
    if (!searchPaths[i]) continue;
    dir = opendir(searchPaths[i]);
    if (dir) { opsDir = searchPaths[i]; break; }
  }
  
  if (!dir) {
    INFO(NCCL_INIT, "NCCL ops directory not found, skipping per-op .so loading");
    return;
  }
  
  INFO(NCCL_INIT, "Loading NCCL ops from %s", opsDir);
  
  struct dirent* ent;
  int loaded = 0;
  while ((ent = readdir(dir)) != NULL) {
    if (strncmp(ent->d_name, "nccl_", 5) != 0) continue;
    if (!strstr(ent->d_name, ".so")) continue;
    
    char path[1024];
    snprintf(path, sizeof(path), "%s/%s", opsDir, ent->d_name);
    void* handle = dlopen(path, RTLD_NOW | RTLD_GLOBAL);
    if (!handle) {
      INFO(NCCL_INIT, "Failed to load %s: %s", path, dlerror());
      continue;
    }
    
    // Call the register function: nccl_xxx.so → ncclOpRegister_xxx
    char regFn[256];
    char name[256];
    ...
    if (dot) *dot = 0;
    snprintf(regFn, sizeof(regFn), "ncclOpRegister_%s", name + 5); // skip "nccl_"
    void* regFunc = dlsym(handle, regFn);
    if (regFunc) {
      ((void(*)())regFunc)();
      loaded++;
    } else {
      dlclose(handle);
      INFO(NCCL_INIT, "No register function %s in %s", regFn, path);
    }
  }
  closedir(dir);
  INFO(NCCL_INIT, "Loaded %d NCCL op modules (%d total kernels)", loaded, ncclDevKernelCount);
}
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  ncclLoadOps();  // Load per-operation .so files
